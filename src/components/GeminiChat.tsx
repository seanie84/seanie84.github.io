...
export default function GeminiChat({ agent }: { agent?: Agent }) {
  const [ready, setReady] = useState(hasEngine());
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      const reply = agent ? await askAgent(agent, text, turns) : await askNexa(text, turns);
    ...
    return (
      <div style={{ padding: '12px 14px', background: 'rgba(34,211,238,0.06)', borderRadius: 8, border: '1px solid var(--line)' }}>
        <p style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--cyan)', letterSpacing: '0.08em', lineHeight: 1.6 }}>
          PICK A FREE ENGINE IN <Link to="/settings" style={{ color: 'var(--cyan)' }}>SETTINGS</Link> TO TALK TO {name.toUpperCase()}.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--cyan)', letterSpacing: '0.1em', marginBottom: 8 }}>
        NEXAS ENGINE · {name.toUpperCase()}
      </div>
      <div style={{ maxHeight: 240, overflowY: 'auto', marginBottom: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
        {turns.map((t, i) => (
          <div key={i} style={{
            alignSelf: t.role === 'user' ? 'flex-end' : 'flex-start',
            maxWidth: '92%', padding: '8px 10px', borderRadius: 10,
            background: t.role === 'user' ? 'rgba(34,211,238,0.12)' : 'var(--glass)',
            border: '1px solid var(--line)', fontFamily: 'Inter', fontSize: 13, color: 'var(--ink)', whiteSpace: 'pre-wrap',
          }}>{t.text}</div>
        ))}
        {busy && <div style={{ fontFamily: 'JetBrains Mono', fontSize: 10, color: 'var(--ink-3)' }}>THINKING…</div>}
        <div ref={end} />
      </div>
      ...
      <form onSubmit={(e) => { e.preventDefault(); void send(); }} style={{ display: 'flex', gap: 8 }}>
        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder={`Ask ${name}…`} style={{
          flex: 1, padding: '9px 12px', background: 'var(--glass)', border: '1px solid var(--line)',
          borderRadius: 8, color: 'var(--ink)', fontFamily: 'Inter', fontSize: 13, outline: 'none',
        }} />
        <button type="submit" disabled={busy} style={{ padding: '9px 14px', borderRadius: 8, border: 'none', background: 'var(--cyan)', color: 'var(--bg)', fontWeight: 700, cursor: 'pointer' }}>Send</button>
      </form>
    </div>
  );
}
