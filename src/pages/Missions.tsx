...
export default function Missions() {
  ...
  async function run() {
    ...
    setErr('');
    try {
      setPlan(await askAmber(brief.trim()));
    } catch (e) {
  ...
  return (
    <div>
      ...
      <p style={{ fontFamily: 'JetBrains Mono', fontSize: 11, color: 'var(--cyan)', letterSpacing: '0.12em', marginBottom: 24 }}>AMBER · GEMINI ORCHESTRATION</p>
      {!hasGeminiKey() && (
        <p style={{ marginBottom: 16, fontFamily: 'Inter', fontSize: 13, color: 'var(--ink-2)' }}>
          Add the Gemini key in <Link to="/settings" style={{ color: 'var(--cyan)' }}>Settings</Link> first.
        </p>
      )}
      <textarea
        value={brief}
        onChange={(e) => setBrief(e.target.value)}
        placeholder="Brief Amber: e.g. prepare a VAT-ready quote pack for a Johannesburg SME"
        rows={5}
        style={{
      ...
      {plan && (
        <pre className="glass" style={{ marginTop: 20, padding: 20, whiteSpace: 'pre-wrap', fontFamily: 'Inter', fontSize: 14, color: 'var(--ink)', maxWidth: 720 }}>{plan}</pre>
      )}
    </div>
  );
}
